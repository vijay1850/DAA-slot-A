...
void dijkstra(int graph[V][V], int src) {
    ...
    for (int count = 0; count < V - 1; count++) {
        ...
        for (int v = 0; v < V; v++) {
            // Update dist[v] only if it's not in sptSet, there's an edge from u to v, and total weight of path from src to v through u is smaller than current value of dist[v]
...
int main() {
    ...
    // Run Dijkstra's algorithm to find the shortest path from vertex 0 to all other vertices
    dijkstra(graph, 0);
